...
#SBATCH --gpus-per-node=a100:2      # Number of GPUs per node to request
#SBATCH --tasks-per-node=2          # Number of processes to spawn per node
#SBATCH --cpus-per-task=12          # Number of CPUs per GPU
#SBATCH --mem=256G                  # Memory per node
#SBATCH --output=../logs/%x_%A-%a_%n-%t.out
#SBATCH --job-name=hl_train_test
